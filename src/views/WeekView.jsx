...
export default function WeekView({ onOpenSettings }) {
  ...
  const pool = useMemo(() => buildPool(saved), [saved])
  const thisWeekId = weekIdFor(new Date())

  const plannedIds = () =>
    new Set(days.map((d) => plan.days[d.iso]?.dinner?.id).filter(Boolean))

  const suggestFor = (iso) => {
    const result = suggestForDay(forecast?.[iso], pool, plannedIds())
    if (!result) return
  ...
  const swapFor = (iso) => {
    ...
    const result = suggestForDay(forecast?.[iso], pool, exclude)
  ...
  const planWholeWeek = () => {
    ...
    for (const d of days) {
      ...
      const result = suggestForDay(forecast?.[d.iso], pool, exclude)
      if (result) {
    ...
    if (added > 0) toast(`Planned ${added} night${added > 1 ? 's' : ''} around the forecast.`)
  }
